...
public class EmailServiceImpl implements EmailService {
    ...
    @Override
    @Async
    public void sendNewSprintNotificationEmail(Sprint sprint) {
        Team team = sprint.getTeam();
        if (team == null) {
            log.error("Sprint ID {} không có team, không thể gửi email thông báo sprint mới.", sprint.getId());
            return;
        }
        ...
        if (recipients.isEmpty()) {
            log.warn("Team ID {} không có thành viên để gửi email thông báo sprint mới.", team.getId());
            return;
        }

        String subject = String.format("Thông báo: Sprint mới '%s' đã được tạo", sprint.getName());
        String htmlContent = getNewSprintNotificationTemplate(sprint);
        ...
        try {
            ...
            mailSender.send(message);
            log.info("Đã gửi email thông báo sprint mới '{}' tới team '{}'", sprint.getName(), team.getName());
        } catch (Exception e) {
            log.error("Lỗi khi gửi email thông báo sprint mới cho team {}: {}", team.getName(), e.getMessage());
        }
    }

    private String getNewSprintNotificationTemplate(Sprint sprint) {
        return """
                <!DOCTYPE html>
                <html lang="vi">
                <head>
                    ...
                    <style>
                        ...
                        .header { background: linear-gradient(135deg, #28a745, #218838); color: #ffffff; padding: 40px 20px; text-align: center; border-radius: 16px 16px 0 0; }
                        .header h1 { font-size: 28px; }
                        .content { padding: 32px 24px; }
                        .details { background-color: #f8f9fa; border-radius: 8px; padding: 20px; margin-bottom: 24px; }
                        .details h2 { font-size: 20px; color: #28a745; margin-bottom: 16px; }
                        .details p { font-size: 16px; line-height: 1.8; margin-bottom: 12px; }
                ...
                <body>
                    <div class="container">
                        <div class="header">
                            <h1>Sprint Mới đã được tạo!</h1>
                        </div>
                        <div class="content">
                            <p>Xin chào team <strong>%s</strong>,</p>
                            <p>Một sprint mới vừa được tạo với các thông tin chi tiết như sau:</p>
                            <div class="details">
                                <h2>%s</h2>
                                <p><strong>Mục tiêu:</strong> %s</p>
                                <p><strong>Thời gian:</strong> Từ <strong>%s</strong> đến <strong>%s</strong></p>
                            </div>
                            <p>Hãy cùng nhau chuẩn bị và hoàn thành tốt các mục tiêu của sprint này nhé!</p>
